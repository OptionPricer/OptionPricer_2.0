...
/**
 * @author Sky The Controller class.
 */
public class OPS {

    /**
     * An arraylist to store the names of basic algorithms chosen by the user.
     */
    ...
    public static ArrayList<Double> customizedResults;
    /**
    ...
    /**
     * To create an option. This method is called when the model.OptionRight and
     * model.OptionStyle is decided,i.e., the Call/Put button is clicked.
     *
    ...
    /**
     * A function to calculate the option price using the String-type formula
     * that user enters. Adapted from the Javaluator v3.0
     *
    ...
    public static boolean isValidateFormula(String str) {
        DoubleEvaluator de = new DoubleEvaluator();
        StaticVariableSet<Double> vars = new StaticVariableSet<>();
        boolean b = false;
        double s = 0;
    ...
    /**
     * A method to calculate option price using all the algorithms that user has
     * chosen.
     */
    public static void compute() {
        ...
        //      For drop in algs
        System.out.println("Number of drop in algs:" + customizedAlgNames.size());
        if (!customizedAlgNames.isEmpty()) {
            System.out.println("There is customized algorithm!");
            customizedResults = new ArrayList<Double>();
            for(int i=0;i<customizedAlgNames.size();i++){
                double temp = evalExpression(customizedAlgNames).get(i);
